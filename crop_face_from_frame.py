...
with open(os.path.join('train.json')) as metadata_json:
    metadata = json.load(metadata_json)



for filename in metadata.keys():
    tmp_path = os.path.join(metadata[filename][list(metadata[filename])[1]])
    print('Processing Directory: ' + tmp_path)
    dir_path = tmp_path.split(os.sep)
    dir_path.pop(len(dir_path)-1)
    dir_path = os.path.join(*dir_path)
    #frame_images = [x for x in os.listdir(tmp_path) if os.path.isfile(os.path.join(tmp_path, x))]
    faces_path = os.path.join(dir_path, 'faces')
    ...
    for result in results:
        ...
        if len(results) < 2 or confidence > 0.95:
            ...
            new_filename = '{}-{:02d}.png'.format(os.path.join(faces_path, get_filename_only(tmp_path)), count)
            count = count + 1
